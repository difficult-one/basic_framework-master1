...
{
    if ((_instance->usart_handle->gState & HAL_UART_STATE_BUSY_TX) == HAL_UART_STATE_BUSY_TX)
        return 0;
    else
        return 1;
}
 ...
 *
 * @param huart 发生中断的串口
 * @param Size 此次接收到的字节数，供模块做变长报文边界检查
 */
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    { // find the instance which is being handled
        ...
        { // call the callback function if it is not NULL
            usart_instance[i]->recv_len = Size > usart_instance[i]->recv_buff_size ?
                                          usart_instance[i]->recv_buff_size : Size;
            if (usart_instance[i]->module_callback != NULL)
            {
                usart_instance[i]->module_callback();
                uint16_t clear_size = Size > usart_instance[i]->recv_buff_size ? usart_instance[i]->recv_buff_size : Size;
                memset(usart_instance[i]->recv_buff, 0, clear_size); // 接收结束后清空buffer,对于变长数据是必要的
            }
