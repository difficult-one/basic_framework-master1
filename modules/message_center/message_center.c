...
static void CheckName(char *name)
{
    ...
    {
        LOGERROR("EVENT NAME INVALID:%s", name ? name : "NULL");
        while (1)
            ; // 进入这里说明话题名超出长度限制
    }
}
...
Publisher_t *PubRegister(char *name, uint8_t data_len)
{
    ...
    {
        LOGERROR("EVENT MALLOC FAILED:%s", name);
        while (1)
            ;
    }
...
Subscriber_t *SubRegister(char *name, uint8_t data_len)
{
    ...
    if (ret == NULL)
    {
        LOGERROR("SUB MALLOC FAILED:%s", name);
        while (1)
            ;
    }
    ...
    { // 给消息队列的每一个元素分配空间,queue里保存的实际上是数据执指针,这样可以兼容不同的数据长度
        ...
        if (ret->queue[i] == NULL)
        {
            LOGERROR("SUB QUEUE MALLOC FAILED:%s", name);
            while (1)
                ;
        }
    }
