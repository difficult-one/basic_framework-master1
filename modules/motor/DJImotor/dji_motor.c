...
static void PIDClearRuntime(PIDInstance *pid)   // 清除PID实例的运行时数据,用于电机初始化和PID重置
{
    if (pid == NULL)
        return;
...
static void DJIMotorClearControllerRuntime(DJIMotorInstance *motor)
{
    if (motor == NULL)
        return;
    ...
    motor->runtime.target_speed = 0;
    motor->runtime.target_current = 0;
    PIDClearRuntime(&motor->motor_controller.angle_PID);
    PIDClearRuntime(&motor->motor_controller.speed_PID);
    PIDClearRuntime(&motor->motor_controller.current_PID);
}
...
void DJIMotorSetRef(DJIMotorInstance *motor, float ref)
{
    ...
    motor->runtime.target_speed = ref;
}

// 为所有电机实例计算三环PID,结果存入output_current,不发送CAN
// CAN发送交由DJIMotorTransmit()完成,以便功率管理器在中间插入限幅
void DJIMotorControl()
{
    ...
    for (size_t i = 0; i < idx; ++i)
    {
        ...
        {
            DJIMotorClearControllerRuntime(motor);
            continue;
        }
        ...
        // 速度环
        if ((motor_setting->close_loop_type & SPEED_LOOP) && (motor_setting->outer_loop_type & (ANGLE_LOOP | SPEED_LOOP)))// 位置环或速度环作为外环,才会进入速度环计算
        {
            if (motor_setting->feedforward_flag & SPEED_FEEDFORWARD)// 若开启速度前馈,则将前馈值加到pid_ref上
                pid_ref += *motor_controller->speed_feedforward_ptr;

            if (motor_setting->speed_feedback_source == OTHER_FEED)// 若速度反馈来源为外部数据,则使用外部数据作为测量值
        ...
        // 存入output_current,供功率管理器读取和限幅
        motor->output_current = pid_ref;

        // 更新实时调试数据
        motor->runtime.target_current = pid_ref;
        motor->runtime.actual_speed   = measure->speed_aps;
        motor->runtime.actual_current = (float)measure->real_current;
    }
}

/**
 * @brief 将所有电机的output_current填入tx_buff并通过CAN发送
 *        应在功率管理器完成限幅后调用
 */
void DJIMotorTransmit()
{
    uint8_t group, num;
    int16_t set;
    DJIMotorInstance *motor;
    ...
    for (size_t i = 0; i < idx; ++i)
    {
        ...
        set = (int16_t)motor->output_current;
