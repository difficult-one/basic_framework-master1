...
/* 电机电流换算系数 */
#define M3508_CURRENT_COEF (819.2f)  // 原始值 → 安培
#define M2006_CURRENT_COEF (1000.0f)
#define GM6020_CURRENT_COEF (1500.0f)

/* 电机电功率数据，用于功率模型拟合和实时监控 */
typedef struct
{
    float electrical_power_w;     // 反馈功率 (W)，用实际电流+转速算
    float predict_power_w;        // 预测功率 (W)，限幅前
    float predict_power_after_w;  // 预测功率 (W)，限幅后
...
typedef struct
{
    ...
    float actual_speed;    // 实际速度 (°/s)
    float target_current;  // 目标电流（原始值）
    float actual_current;  // 实际电流（原始值）
} Motor_Runtime_Data_s;
...
typedef struct
{
    ...
    float output_current; // PID最终输出的电流值,供功率管理器读取/限制

    Motor_Power_Data_s power_data;      // 电机电功率数据,功率管理器更新
    Motor_Runtime_Data_s runtime;       // 电机实时运行数据,供调试打印
} DJIMotorInstance;
