...
/* Control snapshot metadata is not part of the packed referee wire protocol. */
typedef struct
{
    float power_w;
    ...
    uint32_t limit_sample_ms;
    uint8_t power_received;
    uint8_t limit_received;
} RefereePowerSnapshot;

/* Task context only. Timestamps advance only on CRC-checked matching frames. */
