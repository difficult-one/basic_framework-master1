...
static RefereePowerSnapshot referee_power_snapshot;

RefereePowerSnapshot RefereeGetPowerSnapshot(void)
{
	RefereePowerSnapshot snapshot;
	taskENTER_CRITICAL();
	snapshot = referee_power_snapshot;
	taskEXIT_CRITICAL();
	return snapshot;
}
 ...
 */
static void JudgeReadData(uint8_t *buff, uint16_t available)
{
	uint16_t judge_length; // 统计一帧数据长度
	if (buff == NULL || available < LEN_HEADER + LEN_CMDID + LEN_TAIL)
		return;
	...
	if (buff[SOF] == REFEREE_SOF)
	{
		...
		{
			const uint16_t payload_length = referee_info.FrameHeader.DataLength;
			if (payload_length > available - LEN_HEADER - LEN_CMDID - LEN_TAIL)
				return; // A truncated frame must never borrow bytes from an old DMA buffer.
			// 统计一帧数据长度(byte),用于CR16校验
			judge_length = payload_length + LEN_HEADER + LEN_CMDID + LEN_TAIL;
			// 帧尾CRC16校验
			if (Verify_CRC16_Check_Sum(buff, judge_length) == TRUE)
			{
				...
				case ID_game_robot_state: // 0x0201
					if (referee_info.FrameHeader.DataLength != LEN_game_robot_state)
						break;
					memcpy(&referee_info.GameRobotState, (buff + DATA_Offset), LEN_game_robot_state);
					referee_power_snapshot.power_limit_w = referee_info.GameRobotState.chassis_power_limit;
					referee_power_snapshot.limit_sample_ms = HAL_GetTick();
					referee_power_snapshot.limit_received = 1;
					break;
				case ID_power_heat_data: // 0x0202
					if (referee_info.FrameHeader.DataLength != LEN_power_heat_data)
						break;
					memcpy(&referee_info.PowerHeatData, (buff + DATA_Offset), LEN_power_heat_data);
					referee_power_snapshot.power_w = referee_info.PowerHeatData.chassis_power;
					referee_power_snapshot.buffer_energy_j = referee_info.PowerHeatData.buffer_energy;
					referee_power_snapshot.power_sample_ms = HAL_GetTick();
					referee_power_snapshot.power_sequence++;
					referee_power_snapshot.power_received = 1;
					break;
			...
			// Only inspect concatenated frames within this DMA reception.
			if (judge_length < available && buff[judge_length] == REFEREE_SOF)
				JudgeReadData(buff + judge_length, available - judge_length);
		}
	}
}
...
static void RefereeRxCallback()
{
	...
	JudgeReadData(referee_usart_instance->recv_buff, referee_usart_instance->recv_len);
}
