...
modules/algorithm/jam_detector.c \
...
# CXX sources
CXX_SOURCES =  \
modules/powercontrol/power_feedback.cpp \
modules/powercontrol/power_manager.cpp \
modules/powercontrol/power_manager_api.cpp
...
# C++ flags (same as C but with -std=c++17 and -fno-exceptions for embedded)
...
LDFLAGS = $(MCU) -specs=nano.specs -T$(LDSCRIPT) $(LIBDIR) $(LIBS) -Wl,-Map=$(BUILD_DIR)/$(TARGET).map,--cref -Wl,--gc-sections -Wl,--no-warn-rwx-segments -Wl,--print-memory-usage
...
#######################################
clean:
	@if exist $(BUILD_DIR) cmd /c "rd /s/q $(BUILD_DIR)"
...
download_jlink:
	JLink -device STM32F407IG -if SWD -speed 4000 -autoconnect 1 -CommandFile .flash.jlink
