...
本项目是基于RoboMaster开发板C型的示例，MCU为STM32F407IG，使用板载 IMU
BMI088，驱动标准步兵机器人：2 自由度 GM6020 云台、M2006 拨弹盘、2 个 M3508
摩擦轮，以及带超级电容控制器的 4 轮麦克纳姆底盘。当前整机联调配置暂不启用
MG90 弹舱盖舵机。

整机上电默认保持停止。遥控器至少收到一帧有效数据、云台电机反馈在线并完成当前
姿态捕获后，才允许对应机构闭环运行。首次上车必须悬空底盘并保持无弹，按照
`application/chassis/chassis.md` 的 LF、RF、LB、RB 顺序核对轮子 CAN ID 和方向。
摩擦轮的三个速度档目前为保守空载联调值，不等同于实际弹速。
